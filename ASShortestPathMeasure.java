...
public class ASShortestPathMeasure extends AbstractAlignmentSpaceMeasure<LoadedOntology<?>> {
    ...
    public void createIndex(Hashtable<URI, Integer> index, int size) {
    ...
    public void forCicle(int size, int[][] matrix, int it, boolean modified, int max) {
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
    ...
    private void init() throws OntoSimException {
        ...
        this.createIndex(index, size);

        // create matrix
        matrix = new int[size][size];
        // initialise with all

        this.createMatrix(matrix, size);
        ...
        while (modified) {
            it++;
            modified = false;
            this.forCicle(size, matrix, it, modified, max);
        }
